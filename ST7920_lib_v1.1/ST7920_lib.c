...
const char solderingiron[] = {  };
/*-----------------------------Демонстрационное Logo----------------------------*/

/*---------------Вывод стартового демонстрационного лого------------------------*/
void ST7920_logo_demonstration(void) {
	ST7920_Clean_Frame_buffer();
	for (int i = 0; i <= sizeof solderingiron / sizeof solderingiron[0]; i++) {
		Frame_buffer[304 + i] = solderingiron[i];
	}
	ST7920_Update();
	sprintf(tx_buffer, "Saint Petersburg");
	ST7920_Decode_UTF8(16, 40, 1, 0, tx_buffer);
	sprintf(tx_buffer, "electronics lab.");
	ST7920_Decode_UTF8(18, 48, 1, 0, tx_buffer);
	HAL_Delay(500);
	ST7920_Update();
	HAL_Delay(3000);
	sprintf(tx_buffer, "                ");
...
static void ST7920_Graphic_mode(bool enable)   // 1-enable, 0-disable
{
	if (enable) {
...
/*-------------------------Установка позиции в видеобуфере------------------------*/
void ST7920_gotoXY(uint8_t x, uint8_t y) {
	...
	ST7920_Send_command(0x80 | y); // 6-bit (0..63)
	ST7920_Send_command(0x80 | x); // 4-bit (0..15)
}
/*-------------------------Установка позиции в видеобуфере--------------------------*/
...
void ST7920_DrawBitmap(const uint8_t *bmp, int x, uint8_t y, uint8_t w, uint8_t h) {
	...
	for (j8 = 0; j8 < ht8; j8++) {
		for (i = 0; i < w; i++) {
			...
			for (b = 0; b < lastbit; b++) {
				if (d & 1)
					Frame_buffer[(y + j8 * 8 + b) * scrWd + (x + i) / 8] |= mask;
				d >>= 1;
			}
		}
	}
...
void ST7920_Draw_pixel(uint8_t x, uint8_t y, uint8_t col) {
	...
	switch (col) {
	case 1:
		Frame_buffer[y * scrWd + x / 8] |= (0x80 >> (x & 7)); //Закрасить пиксель
		break;
	case 0:
		Frame_buffer[y * scrWd + x / 8] &= ~(0x80 >> (x & 7)); //Очистить пиксель
		break;
	case 2:
		Frame_buffer[y * scrWd + x / 8] ^= (0x80 >> (x & 7)); //Инвертировать пиксель
		break;
	}
}
/*------------------------Функция рисования пикселя-------------------------*/
...
void ST7920_Update(void) {
	...
	for (j = 0; j < 64 / 2; j++) {
		...
		for (i = 0; i < 16; i++) {  // 16 bytes from line #0+
			b = Frame_buffer[i + j * 16];
			ST7920_Send_data(b);
		}
		for (i = 0; i < 16; i++) {  // 16 bytes from line #32+
			b = Frame_buffer[i + (j + 32) * 16];
			ST7920_Send_data(b);
		}
	}
}
	...
	for (uint8_t x = x_start; x <= x_stop; x++) {
		if (x == x_stop) { //Заполняем межсимвольные интервалы
			for (uint8_t i = 0; i <= 6; i++) { // от 0 до 6, т.к. шрифт высотой 7 пикселей
				if (0x00 & (1 << i)) {
					if (inversion) {
						ST7920_Draw_pixel(x, y + i, 0); //Закрасить пиксель
					} else {
						ST7920_Draw_pixel(x, y + i, 1); //Закрасить пиксель
					}
				} else {
					if (inversion) {
						ST7920_Draw_pixel(x, y + i, 1); //Очистить пиксель
					} else {
						ST7920_Draw_pixel(x, y + i, 0); //Очистить пиксель
					}
				}
			}
		...
		if (tx_buffer[i] < 0xC0) { //Английский текст и символы. Если до русского текста, то [i] <0xD0. Но в font добавлен знак "°"
			if (flag_block) {
				...
				if (font == font3x5) { //Если выбран шрифт размера 3x5
					if (inversion) {
						ST7920_Print_symbol_3x5(x, y, symbol - 32, 1); //Таблица UTF-8. Basic Latin. С "пробел" до "z". Инверсия вкл.
					} else {
						ST7920_Print_symbol_3x5(x, y, symbol - 32, 0); //Таблица UTF-8. Basic Latin. С "пробел" до "z". Инверсия выкл.
					}
					x = x + 4;
				} else if (font == font5x7) { //Если выбран шрифт размера 5x7
					if (inversion) {
						ST7920_Print_symbol_5x7(x, y, symbol - 32, 1); //Таблица UTF-8. Basic Latin. С "пробел" до "z". Инверсия вкл.
					} else {
						ST7920_Print_symbol_5x7(x, y, symbol - 32, 0); //Таблица UTF-8. Basic Latin. С "пробел" до "z". Инверсия выкл.
					}
					x = x + 6;
				}
			}
			...
			if (symbol < 0xD180 && symbol > 0xD081) {
				if (font == font3x5) { //Если выбран шрифт размера 3x5
					if (inversion) {
						ST7920_Print_symbol_3x5(x, y, symbol - 53297, 1); //Таблица UTF-8. Кириллица. С буквы "А" до "п". Инверсия вкл.
					} else {
						ST7920_Print_symbol_3x5(x, y, symbol - 53297, 0); //Таблица UTF-8. Кириллица. С буквы "А" до "п". Инверсия выкл.
					}
					x = x + 4;
				} else if (font == font5x7) { //Если выбран шрифт размера 5x7
					if (inversion) {
						ST7920_Print_symbol_5x7(x, y, symbol - 53297, 1); //Таблица UTF-8. Кириллица. С буквы "А" до "п". Инверсия вкл.
					} else {
						ST7920_Print_symbol_5x7(x, y, symbol - 53297, 0); //Таблица UTF-8. Кириллица. С буквы "А" до "п". Инверсия выкл.
					}
					x = x + 6;
				}
				...
				if (font == font3x5) { //Если выбран шрифт размера 3x5
					if (inversion) {
						ST7920_Print_symbol_3x5(x, y, 159, 1); ////Таблица UTF-8. Кириллица. Буква "Ё". Инверсия вкл.
					} else {
						ST7920_Print_symbol_3x5(x, y, 159, 0); ////Таблица UTF-8. Кириллица. Буква "Ё". Инверсия выкл.
					}
					x = x + 4;
				} else if (font == font5x7) { //Если выбран шрифт размера 5x7
					if (inversion) {
						ST7920_Print_symbol_5x7(x, y, 159, 1); ////Таблица UTF-8. Кириллица. Буква "Ё". Инверсия вкл.
					} else {
						ST7920_Print_symbol_5x7(x, y, 159, 0); ////Таблица UTF-8. Кириллица. Буква "Ё". Инверсия выкл.
					}
					x = x + 6;
				}
				...
				if (font == font3x5) { //Если выбран шрифт размера 3x5
					if (inversion) {
						ST7920_Print_symbol_3x5(x, y, 160, 1); ////Таблица UTF-8. Кириллица. Буква "ё". Инверсия вкл.
					} else {
						ST7920_Print_symbol_3x5(x, y, 160, 0); ////Таблица UTF-8. Кириллица. Буква "ё". Инверсия выкл.
					}
					x = x + 4;
				} else if (font == font5x7) { //Если выбран шрифт размера 5x7
					if (inversion) {
						ST7920_Print_symbol_5x7(x, y, 160, 1); ////Таблица UTF-8. Кириллица. Буква "ё". Инверсия вкл.
					} else {
						ST7920_Print_symbol_5x7(x, y, 160, 0); ////Таблица UTF-8. Кириллица. Буква "ё". Инверсия выкл.
					}
					x = x + 6;
				}
				...
				if (font == font3x5) { //Если выбран шрифт размера 3x5
					if (inversion) {
						ST7920_Print_symbol_3x5(x, y, 161, 1); ////Таблица UTF-8. Basic Latin. Символ "°". Инверсия вкл.
					} else {
						ST7920_Print_symbol_3x5(x, y, 161, 0); ////Таблица UTF-8. Basic Latin. Символ "°". Инверсия выкл.
					}
					x = x + 4;
				} else if (font == font5x7) { //Если выбран шрифт размера 5x7
					if (inversion) {
						ST7920_Print_symbol_5x7(x, y, 161, 1); ////Таблица UTF-8. Basic Latin. Символ "°". Инверсия вкл.
					} else {
						ST7920_Print_symbol_5x7(x, y, 161, 0); ////Таблица UTF-8. Basic Latin. Символ "°". Инверсия выкл.
					}
					x = x + 6;
				}
			}
				...
				if (font == font3x5) { //Если выбран шрифт размера 3x5
					if (inversion) {
						ST7920_Print_symbol_3x5(x, y, symbol - 53489, 1); //Таблица UTF-8. Кириллица. С буквы "р", начинается сдвиг. Инверсия вкл.
					} else {
						ST7920_Print_symbol_3x5(x, y, symbol - 53489, 0); //Таблица UTF-8. Кириллица. С буквы "р", начинается сдвиг. Инверсия выкл.
					}
					x = x + 4;
				} else if (font == font5x7) { //Если выбран шрифт размера 5x7
					if (inversion) {
						ST7920_Print_symbol_5x7(x, y, symbol - 53489, 1); //Таблица UTF-8. Кириллица. С буквы "р", начинается сдвиг. Инверсия вкл.
					} else {
						ST7920_Print_symbol_5x7(x, y, symbol - 53489, 0); //Таблица UTF-8. Кириллица. С буквы "р", начинается сдвиг. Инверсия выкл.
					}
					x = x + 6;
				}
			}
...
/// \param float value - значение, которое будем отображать на графике
	...
	if (y_max > y_min && value >= y_min && value <= y_max) {
		...
		ST7920_Decode_UTF8(37, 26, 1, 0, text);
		ST7920_Update();
	} else if (value < y_min) {
		Graph_value = 0;
		sprintf(text, "   Clipping    ");
...
/// \param uint8_t value - подготовленное значение для графика(подготавливается при помощи ф-ии uint8_t ST7920_Value_for_Plot(int y_min, int y_max, float value))
	if (*counter == 0) {
		array[0] = value;
	}
...
void ST7920_Generate_a_Graph(uint8_t *counter, uint8_t *array, uint8_t size_array, int y_min, int y_max, uint8_t x_grid_time, uint8_t time_interval,
bool grid) {
...
/// \param bool grid - Сетка граффика. вкл = 1. выкл = 0.
	...
	ST7920_Decode_UTF8(8, 48, 0, 0, text);
	sprintf(text, "%4.1d", (y_min + val_del));
	ST7920_Decode_UTF8(8, 38, 0, 0, text);
	sprintf(text, "%4.1d", (y_min + val_del * 2));
	ST7920_Decode_UTF8(8, 28, 0, 0, text);
	sprintf(text, "%4.1d", (y_min + val_del * 3));
	ST7920_Decode_UTF8(8, 18, 0, 0, text);
	sprintf(text, "%4.1d", (y_min + val_del * 4));
	ST7920_Decode_UTF8(8, 8, 0, 0, text);
	sprintf(text, "%4.1d", y_max);
	ST7920_Decode_UTF8(8, 0, 0, 0, text);
	...
	ST7920_Decode_UTF8(53, 57, 1, 0, text);

	/*----------------Ось асцисс, ось ординат, разметка-----------------*/

	ST7920_Draw_line(27, 50, 127, 50, 1); //ось абсцисс
	for (uint8_t i = 27; i <= 77; i = i + 10) {
		ST7920_Draw_line(i, 51, i, 52, 1);
	}
	for (uint8_t i = 87; i <= 127; i = i + 10) {
		ST7920_Draw_pixel(i, 51, 1);
	}

	ST7920_Draw_line(27, 0, 27, 50, 1); //ось ординат
	for (uint8_t i = 0; i <= 50; i = i + 10) {
		ST7920_Draw_line(25, i, 27, i, 1);
	}

	ST7920_Draw_line(57, 53, 57, 55, 1); //Полоса по х левая
	ST7920_Draw_line(67, 53, 67, 55, 1); //Полоса по х правая
	ST7920_Draw_line(52, 54, 72, 54, 1); //Полоса между делениями по х
	ST7920_Draw_line(54, 52, 54, 56, 1); //Левая стрелка на деление
	ST7920_Draw_line(55, 53, 55, 55, 1); //Левая стрелка на деление
	ST7920_Draw_line(69, 53, 69, 55, 1); //Правая стрелка на деление
	ST7920_Draw_line(70, 52, 70, 56, 1); //Правая стрелка на деление
	ST7920_Draw_line(127, 53, 127, 57, 1); //Отметка "сейчас"
	ST7920_Draw_line(122, 55, 127, 55, 1); //Стрелка на "сейчас"
	ST7920_Draw_line(124, 53, 124, 57, 1); //Стрелка на "сейчас"
	ST7920_Draw_line(125, 54, 125, 56, 1); //Стрелка на "сейчас"
	...
	if (array_is_full) {
		for (int i = 0; i < size_array - 1; i++) {
			//ST7920_Draw_pixel(127 - i, 50 - array[i], 1); //Рисуем точками
			ST7920_Draw_line(127 - (i + 1), 50 - array[i + 1], 127 - i, 50 - array[i], 1); //Рисуем линиями
			//ST7920_Draw_line(127-i, 50, 127-i, 50 - array[i], 1);//Закрашиваем область
...
void ST7920_Draw_line(uint8_t x_start, uint8_t y_start, uint8_t x_end, uint8_t y_end, uint8_t color) {
	...
	for (;;) {
		ST7920_Draw_pixel(x_start, y_start, color);
...
/// \param radius - радиус окружности
	...
	ST7920_Draw_pixel(x, y + radius, color);
	ST7920_Draw_pixel(x, y - radius, color);
	ST7920_Draw_pixel(x + radius, y, color);
	ST7920_Draw_pixel(x - radius, y, color);
	...
	while (x_0 < y_0) {
		...
		ddF_x += 2;
		f += ddF_x;
		ST7920_Draw_pixel(x + x_0, y + y_0, color);
		ST7920_Draw_pixel(x - x_0, y + y_0, color);
		ST7920_Draw_pixel(x + x_0, y - y_0, color);
		ST7920_Draw_pixel(x - x_0, y - y_0, color);
		ST7920_Draw_pixel(x + y_0, y + x_0, color);
		ST7920_Draw_pixel(x - y_0, y + x_0, color);
		ST7920_Draw_pixel(x + y_0, y - x_0, color);
		ST7920_Draw_pixel(x - y_0, y - x_0, color);
	}
